...
Data Generation:
To create our large test database, we created a python script. In our repo, in our main folder there is a file called dataGenerator.py which is the script. To generate the data, you can run this file and then go to the folder db/data (we have already done this). Here you will see a bunch of CSV files. The ones with the suffix “Generated” were generated by our python script. Then in the db folder in the load.sql file we populate our database by loading the generated csv files to our database. We used the \Copy function to load the files into the database.
